...
| **`9002`** | `annotation-sop-visualizer` | Trực quan hóa quy trình SOP, máy tính Wilson Score, K-100 & tạo biên bản nghiệm thu |
...
1. **Nhật ký tuần (`nhat-ky-tuan/tuan-XX.md`):**
   - **Nguyên tắc Append-only & Timestamp:** Khi người dùng cập nhật công việc, luôn thêm dòng mới vào cuối Daily Log (`2.2`), **tuyệt đối không ghi đè** lên các dòng nhật ký cũ. Bắt buộc có mốc thời gian ngày giờ cập nhật (`- **dd/mm/yyyy - HH:MM:**`).
   - **Phạm vi cập nhật:** Chỉ cập nhật tiến độ ở các phần tổng hợp (Bảng 1, Mục 2.1, và Mục 5); giữ nguyên toàn bộ lịch sử các dòng nhật ký cũ.
   - **Tính toán % tiến độ chuẩn hóa:** Bắt buộc tính toán định lượng % tiến độ dựa trên **Tổng số lượng ảnh của Job** và **Số lượng label quy định trong Guideline** ($N_{ảnh} \times K_{nhãn}$). Khi người dùng báo cáo công việc, AI tự động tính toán lại % tiến độ của job và tổng thể rồi cập nhật đồng bộ vào các phần tổng hợp.
   - **Biểu tượng chuẩn:**
     - `✅ 100%`: Hoàn thành & đã qua review.
     - `🟡 xx%`: Đang làm (% tiến độ định lượng).
...
4. **Sổ Pain Points (`pain-points.md`):**
   - Mã `PP-NNN` tăng dần (không tái sử dụng mã đã hủy).
   - 5 phân loại: *🛠️ Công cụ & Hạ tầng*, *📘 Đặc tả & Guideline*, *🩺 Thể chất & Thao tác*, *⚖️ Review & Phản hồi*, *📷 Chất lượng Dữ liệu*.
   - 4 mức độ: *🚨 Nghiêm trọng*, *⚠️ Cao*, *⚡ Trung bình*, *💡 Thấp*.
   - 4 trạng thái: `🔴 Đang gặp`, `🟡 Giải pháp tạm` (ghi rõ Workaround), `🛠️ Đang làm tool` (trỏ `source-tool/`), `✅ Đã giải quyết` (trỏ Tool hoặc QĐ).
5. **Sổ Ý Tưởng Công Cụ (`source-tool/tool-ideas.md`):**
   - Lưu trữ các đề xuất phát triển tool nhằm tháo gỡ pain point của annotator.
   - 4 thành phần bắt buộc: **Thứ tự ý tưởng** (`STT` / mã `IDEA-NNN`), **Nội dung** (Tên tool, Mô tả giải pháp kỹ thuật, Hiệu quả kỳ vọng), **Thời gian đề xuất** (`dd/mm/yyyy - HH:MM`), và **Link tới Pain Point ID** (`[PP-xxx](../pain-points.md#pp-xxx)`).
   - 5 trạng thái: `💡 Ý tưởng`, `🛠️ Đang thiết kế`, `🚀 Đang phát triển`, `✅ Đã triển khai`, `⚪ Tạm hoãn`.
...
- AI Agent và thành viên sử dụng skill này khi cần cập nhật `problem-backlog.md`, `so-quyet-dinh.md`, `pain-points.md` và `nhat-ky-tuan/tuan-NN.md`.
- Bộ cẩm nang mẫu chi tiết đi kèm:
  ...
  - [Sổ Pain Points Reference](.agents/skills/annotation-reporting/references/pain-points-guide.md)
