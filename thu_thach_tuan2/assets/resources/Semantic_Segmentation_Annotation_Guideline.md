...
- **Dữ liệu:** ảnh `.jpg` trong thư mục `segmentation/images`. Học viên không sử dụng annotation/ground truth có sẵn khi thực hành.
...
- Chỉ sử dụng **19 class** đã quy định trong tài liệu này. Không tự tạo class, không đổi tên class, không ghép class theo cảm tính.
...
> **Lưu ý:** Bảng trên là palette hiển thị của bài tập. Khi cấu hình CVAT, nên đặt màu label tương ứng để overlay nhất quán giữa các nhóm.
...
2. Dùng **Mask/Brush** cho vùng pixel phức tạp; có thể dùng **Polygon** cho vùng lớn có biên tương đối rõ nếu workflow của lớp cho phép.
3. Ưu tiên annotate vùng lớn trước (`road`, `sky`, `building`, `vegetation`), sau đó tới object nhỏ/mảnh.
...
6. Nếu không chắc class/boundary, tạo Issue hoặc đánh dấu theo SOP review; không tự đặt quy tắc mới.
